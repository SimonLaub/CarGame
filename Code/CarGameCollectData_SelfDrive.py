...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 19 12:17:31 2024
...
pygame.display.set_caption("Car Game with Curving Road and Scoring. Collecting Data.")
...
USER_ACTION_NONE = 0
USER_ACTION_RIGHT =1
Store_user_action = USER_ACTION_NONE

#Store user action
def store_user_action(user_action):
    global Store_user_action
    if keys[pygame.K_LEFT]:
        Store_user_action = USER_ACTION_LEFT
    elif keys[pygame.K_RIGHT]:
        Store_user_action = USER_ACTION_RIGHT
    else:
        Store_user_action = USER_ACTION_NONE
...
def render_collected_data(calculated_car_offset):
    ...
    user_pressed_text = font.render(f"Used Pressed: {Store_user_action}", True, BLACK)
    window.blit(user_pressed_text, (10, 150))  # Draw car offset at top-left corner

    where_is_traffic_text = font.render(f"Where is Traffic: {Store_where_is_traffic}", True, BLACK)
    window.blit(where_is_traffic_text, (10, 200))  # Draw car offset at top-left corner
...
def store_collected_data():
    ...
    if SELF_DRIVE == False:
       Store_Data.append({
          'Car_Offset': Store_car_offset,
           'Traffic_Position': Store_where_is_traffic,
           'User_action': Store_user_action
       })
    else:
       Store_Data.append({
         'Car_Offset': Store_car_offset,
         'Traffic_Position': Store_where_is_traffic,
         'User_action': WHEEL_TURN
       })
...
while running:
    ...
    car_offset = calc_car_offset(car_rect.left + (car_width // 2), road_x_at_car + (road_width // 2))

    if (SELF_DRIVE == True):
        if (Store_where_is_traffic == TRAFFIC_IN_LEFT_LANE):
           car_offset = calc_car_offset(car_rect.left + (car_width // 2), road_x_at_car + 140)
        else:
           car_offset = calc_car_offset(car_rect.left + (car_width // 2), road_x_at_car + 40)
        if (car_offset<0):
           car_x += car_speed
        else:
           car_x -= car_speed

        store_self_drive(car_rect.left + (car_width // 2), road_x_at_car + (road_width // 2))
        # Update car position
        car_rect = pygame.Rect(car_x, car_y, car_width, car_height)
...
# Save the DataFrame to an Excel file using a context manager
with pd.ExcelWriter('game_data.xlsx') as writer:
